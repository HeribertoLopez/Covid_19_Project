...
ui = fluidPage(
  ...
  output$percentChangeVax = renderPlot({
    ggplot(
      ...
      geom_point(mapping = aes(x = vaxChange1, y = new_cases_per_100k, color = "People Vaccinated"),size=4, shape = 20, na.rm=TRUE) + 
      geom_point(mapping = aes(x = vaxChange2, y = new_cases_per_100k, color = "People Fully Vaccinated"),size=4, shape = 1, na.rm=TRUE) + 
      xlab("Vaccination Rate") + ylab("Cases per 100k people") + ggtitle("% Change in COVID-19 Vaccinations vs. Cases") +
      labs(color = "") + theme_bw() 
  })
