...
# get order for max percent fully faccinated
state.ordered <- dat %>%
  select(state, fully_vax_start) %>%
  group_by(state) %>%
  summarise(max.vax = max(fully_vax_start, na.rm=T)) %>%
  arrange(max.vax)

dat$state <- factor(dat$state, levels=state.ordered$state)
dat$fully_vax_start <- as.factor(dat$fully_vax_start)

dat %>%
  complete(fully_vax_start, nesting(state)) %>%
  ggplot(aes(x=as.factor(fully_vax_start), y=state, fill=rate)) +
  geom_tile(color = "white", size = 0.35) +
  scale_fill_gradientn(colors = brewer.pal(9, 'Reds'), na.value='gray') +
  theme_minimal() + 
  theme(panel.grid = element_blank(), axis.text.x = element_text(angle=90, hjust=1)) +
  coord_cartesian(clip = 'off') +
  ggtitle("COVID19 US States") +
  ylab("") +
