...
PercentCleanWeekData = CleanWeekData %>%
  mutate(FullyVaccChange = (percent_ppl_fully_vacc-lag(percent_ppl_fully_vacc))/lag(percent_ppl_fully_vacc)*100, 
         VaccChange = (percent_ppl_vacc-lag(percent_ppl_vacc))/lag(percent_ppl_vacc)*100, 
         PercentCaseChange = (new_cases_per_100k-lag(new_cases_per_100k)/lag(new_cases_per_100k))*100)
PercentCleanWeekData[mapply(is.infinite, PercentCleanWeekData)] <- NA
library(shiny)
...
server = function(input, output){ 
  output$line = renderPlot( {
    ggplot(PercentCleanWeekData %>%  
             filter(location == input$location)) + 
      geom_point(mapping = aes(y = PercentCaseChange, x = FullyVaccChange, color = "Change in Fully Vaccinated"), na.rm=TRUE) +
      geom_point(mapping = aes(y = PercentCaseChange, x = VaccChange, color = "Change in Vaccinated"), na.rm=TRUE) +
      xlab("Change in Vaccinations [%]") + ylab("Change in New Weekly Cases [%]") + ggtitle("COVID-19 Panedemic") +
      labs(color = "") + theme_bw() + xlim(-10, 70) + ylim(-100, 6000)
  }
  )
}
# Run the application 
