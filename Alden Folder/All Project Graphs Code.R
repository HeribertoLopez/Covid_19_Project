...
server = function(input, output){ 
  ...
  output$CasesVax = renderPlot( {
    ggplot(CleanWeekData %>%  
             filter(location == input$location)) + 
      geom_point(mapping = aes(x = percent_ppl_fully_vacc, color = "People Fully Vaccinated [Weekly %]", y= new_cases_per_100k), na.rm=TRUE) +
      geom_point(mapping = aes(x = percent_ppl_vacc, color = "People Vaccinated [Weekly %]", y = new_cases_per_100k), na.rm=TRUE) +
      xlab("Vaccinations") + ylab("Cases Per 100,000") +
      ggtitle("COVID-19 Pandemic") +
      labs(color = "") + theme_bw()  
  }
  )
